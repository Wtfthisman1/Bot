...
public interface HomeApi {
    ...
    /**
     * Одноразовая ссылка, по которой этот чат входит на сайт.
     *
     * <p>Аккаунты живут дома, а просьба приходит из чата, то есть со стороны
     * бота. Ссылку выдаёт дом и он же её потом гасит.</p>
     *
     * <p>Направление важнее самой ссылки: раньше вход начинала страница, а
     * подтверждали его в чате — и достаточно было прислать постороннему
     * ссылку на бота, чтобы он подтвердил чужой вход. Теперь адрес рождается
     * из чата человека и приходит только в этот чат, поэтому дать чужому
     * нечего.</p>
     *
     * @return адрес для браузера; пусто — ссылок просят слишком часто
     */
    Optional<String> loginLink(long chatId, String displayName);
    ...
    enum Acceptance {
        ...
        /**
         * У этого владельца и так полно незавершённых задач.
         *
         * <p>Очередь одна на всех и разбирается по порядку поступления, а
         * скачивание квоты не тратит вовсе. Без этого предела один чат мог
         * поставить хоть тысячу загрузок и занять видеокарту и очередь всем
         * остальным на сутки вперёд — не нарушив при этом ни одного правила.</p>
         */
        TOO_MANY_ACTIVE("⏳ Слишком много задач уже в работе. Дождитесь, пока "
                + "закончатся начатые, — очередь общая."),
        /**
         * Место, отведённое владельцу под записи, кончилось.
         *
         * <p>Лимит считает расшифровки, а не гигабайты, и скачивание не считает
         * вовсе: без отдельной проверки места один чат забивал диск домашней
         * машины ссылками на длинные ролики.</p>
         */
        NO_ROOM("💾 Кончилось место, отведённое под ваши записи. Старые уносит "
                + "ночная чистка через две недели.");
        ...
        public boolean isRejected() {
            return this == QUOTA_EXCEEDED || this == UNSUPPORTED
                    || this == TOO_MANY_ACTIVE || this == NO_ROOM;
        }
